...
The mean number of steps per day is `r steps_by_day_mean` and the median number of steps per day is `r steps_by_day_median`.
...
The 5-minute interval with the highest average number of steps is `r max_interval` to `r max_interval_5`. 
...
There are `r Total_NAs` missing values in the dataset.
...
The mean number of steps per day for the completed dataset is `r steps_by_day_mean2` and the median is `r steps_by_day_median2`. 

The original values were `r steps_by_day_mean` mean and `r steps_by_day_median`
